  ...
  return (
    <div className="app-page">
      ...
      <div className="two-col">
        <div className="card card-pad-lg"><div className="section-title">Compliance queue</div>{[["Sarah Johnson", "Right to Work review"], ["Amina Hassan", "QTS verification"], ["Northfield Academy", "School onboarding"]].map(([name, task]) => <div key={name} className="flex items-center justify-between border-b border-[var(--border)] py-3"><div><div className="font-medium">{name}</div><div className="text-xs text-[var(--muted)]">{task}</div></div><Btn size="sm">Open</Btn></div>)}</div>
        <div className="card card-pad-lg"><div className="section-title">Trust metrics</div>{[["DBS verified", "99.2%"], ["Teacher response within 10m", "81%"], ["Disputes this month", "0.7%"], ["Average rating", "4.9"]].map(([label, value]) => <div key={label} className="flex items-center justify-between border-b border-[var(--border)] py-2.5"><span>{label}</span><span className="font-semibold">{value}</span></div>)}</div>
      </div>
    </div>
  );
}
