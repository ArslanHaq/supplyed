        ...
        <div className="two-col">
          <div>
            <div className="mb-3.5 flex items-center justify-between">
              ...
              <span className="cursor-pointer text-xs font-semibold text-[var(--se)]" onClick={() => go("applications")}>View all →</span>
            </div>
            <div className="card overflow-hidden">
              {seedJobs.map((job) => (
                <div key={job.id} className="flex cursor-pointer items-center gap-4 border-b border-[var(--border)] px-5 py-4" onClick={() => go("applications", { jobId: job.id })}>
                  <div className="flex-1">
                    <div className="mb-1 flex items-center gap-2">{job.urgent ? <Tag tone="red">Urgent</Tag> : null}<Tag tone={job.mode === "instant" ? "" : "purple"}>{job.mode === "instant" ? "Instant" : "Brief"}</Tag><span className="text-xs text-[var(--muted)]">{job.postedAt}</span></div>
                    <div className="text-[15px] font-semibold">{job.title}</div>
                    <div className="text-xs text-[var(--muted)]">{job.school} - {job.date} - £{job.rate}/day</div>
                  </div>
                  <div className="text-center"><div className="font-serif text-[22px] text-[var(--se)]">{job.applicants}</div><div className="text-xs text-[var(--muted)]">Applicants</div></div>
                </div>
              ))}
            </div>
            ...
            <div className="card card-pad">
              {[
                ...
                <div key={entry} className="flex items-center justify-between border-b border-[var(--border)] py-2.5">
                  <div>{entry}</div>
          ...
          <div>
            ...
            <div className="card overflow-hidden">
              {seedTeachers.slice(0, 4).map((teacher, index) => (
                <div key={teacher.id} className="flex cursor-pointer items-center gap-2.5 border-b px-3.5 py-3" style={{ borderBottomColor: index < 3 ? "var(--border)" : "transparent" }} onClick={() => go("teacher-profile", { teacherId: teacher.id })}>
                  ...
                  <div className="flex-1"><div className="font-medium">{teacher.name}</div><div className="text-xs text-[var(--muted)]">{teacher.role}</div></div>
            ...
            <div className="card card-pad border-[var(--se)] bg-[var(--se-tint)]">
              ...
              <div className="mt-1 text-xs text-[var(--muted)]">Renews 24 Apr 2026</div>
