    ...
    <div className="app-page">
      ...
      <div className="two-col">
        <div className="card card-pad-lg">
          ...
          <div className="text-[var(--muted)]">Unlimited job posts, compliance tools, messaging, and AI matching.</div>
        ...
        <div className="card card-pad-lg">
          ...
          {[["INV-2041", "Mar 2026", "£99"], ["INV-1978", "Feb 2026", "£99"], ["INV-1922", "Jan 2026", "£99"]].map(([id, month, amount]) => (
            <div key={id} className="flex items-center justify-between border-b border-[var(--border)] py-3"><div><div className="font-medium">{id}</div><div className="text-xs text-[var(--muted)]">{month}</div></div><div className="flex items-center gap-2"><span>{amount}</span><Btn variant="ghost" size="sm" icon="download">PDF</Btn></div></div>
          ))}
        </div>
      </div>
    </div>
