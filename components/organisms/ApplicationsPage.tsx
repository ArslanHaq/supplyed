  ...
  return (
    <div className="app-page">
      ...
      <div className="card card-pad mb-6 flex flex-wrap items-center gap-5">
        ...
        <div><div className="text-xs text-[var(--muted)]">Applicants</div><div className="font-serif text-[22px]">{apps.length}</div></div>
        <div><div className="text-xs text-[var(--muted)]">Shortlisted</div><div className="font-serif text-[22px]">{apps.filter((item) => item.stage === "shortlisted").length}</div></div>
            ...
            <div key={stage} className="kanban-col">
              ...
              {apps.filter((item) => item.stage === stage).map((application) => (
                <div key={application.id} className="kanban-card" onClick={() => go("teacher-profile", { teacherId: application.teacher.id })}>
                  <div className="mb-2 flex items-center gap-2"><Avatar name={application.teacher.name} size="sm" tone={application.teacher.tone} /><div className="flex-1"><div className="font-medium">{application.teacher.name}</div><div className="text-xs text-[var(--muted)]">{application.teacher.role}</div></div><MatchScore score={application.teacher.matchScore} size={32} /></div>
                  <div className="mb-2 text-xs text-[var(--muted)]">{`"${application.coverLetter.slice(0, 72)}..."`}</div>
                  <div className="flex items-center justify-between"><span className="text-xs font-semibold text-[var(--se)]">£{application.rate}/day</span><span className="text-xs text-[var(--muted)]">{application.appliedAt}</span></div>
                </div>
              ))}
            </div>
          ))}
            ...
            <tbody>
              {apps.map((application) => (
                <tr key={application.id}>
                  <td><div className="flex items-center gap-2.5"><Avatar name={application.teacher.name} size="sm" tone={application.teacher.tone} /><div><div className="font-medium">{application.teacher.name}</div><div className="text-xs text-[var(--muted)]">{application.teacher.role}</div></div></div></td>
